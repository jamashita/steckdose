...
describe('Config', () => {
  describe('get', () => {
    it.each`
      ...
      ${'l'}     | ${128.2}
      ${'m'}     | ${null}
    `('load test.json when json file exists, json is prioritised, key is $key', ({ key, expected }: { key: string; expected: unknown }) => {
    ...
    it.each`
      ...
      ${'l'}       | ${{ m: 788.64 }}
      ${'l.m'}     | ${788.64}
    `('load test.yml when json file does not exist, yml is prioritised, key is $key', ({ key, expected }: { key: string; expected: unknown }) => {
    ...
    `(
      ...
      ({ key, expected }: { key: string; expected: unknown }) => {
